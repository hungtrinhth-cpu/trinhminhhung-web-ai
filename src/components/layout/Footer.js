    ...
    <footer className="w-full section-blue-banner text-pure-white">
      ...
      <div className="border-b border-white/10">
        <div className="max-w-7xl mx-auto px-container-padding-mobile md:px-container-padding-desktop py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <p className="font-headline-sub text-headline-sub text-pure-white">
              {f.newsletter_title ?? "Đăng ký nhận kiến thức AI miễn phí"}
            </p>
            <p className="font-body-md text-white/70 mt-1">
              {f.newsletter_sub ?? "Mỗi tuần một bài học AI thực chiến, gửi thẳng vào hộp thư của bạn."}
            </p>
          </div>
          <div className="relative w-full md:w-[380px] shrink-0">
            <input
              ...
              placeholder={f.newsletter_placeholder ?? "Email của bạn"}
              type="email"
            />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="space-y-5">
            ...
            <p className="font-body-md text-white/70 leading-relaxed max-w-xs">
              {f.brand_desc ?? "Kiến tạo tương lai cùng trí tuệ nhân tạo."}
            </p>
          ...
          <div className="space-y-4">
            <h4 className="font-button-text text-button-text text-sunrise uppercase tracking-widest">
              {f.col_services ?? "Dịch vụ"}
            </h4>
            {(f.services ?? ["Đào tạo AI nội bộ", "Chuyển giao công nghệ", "Tư vấn chiến lược AI", "Xây dựng trợ lý AI"]).map((label, i) => (
              <Link
                key={i}
                href={serviceHrefs[i] ?? `/${lang}/khoa-hoc`}
          ...
          <div className="space-y-4">
            <h4 className="font-button-text text-button-text text-sunrise uppercase tracking-widest">
              {f.col_support ?? "Hỗ trợ"}
            </h4>
            {(f.support ?? ["Chính sách bảo mật", "Điều khoản dịch vụ", "Liên hệ hỗ trợ", "FAQ"]).map((label, i) => (
              <Link
                key={i}
                href={supportHrefs[i] ?? `/${lang}/contact`}
                className="block font-body-md text-white/70 hover:text-sunrise transition-colors"
              >
                {label}
              </Link>
          ...
          <div className="space-y-4">
            <h4 className="font-button-text text-button-text text-sunrise uppercase tracking-widest">
              {f.col_contact ?? "Hung Trinh AI"}
            </h4>
            {[
              { icon: "location_on", key: "address", fallback: "Hà Nội, Việt Nam" },
              { icon: "mail", key: "email", fallback: "contact.visun@gmail.com" },
              { icon: "phone", key: "phone", fallback: "0986 315 286" },
              { icon: "schedule", key: "hours", fallback: "Thứ 2 – Thứ 6, 8:00 – 17:00" },
            ].map((item) => (
              <div key={item.key} className="flex items-start gap-3">
                <span className="material-symbols-outlined text-sunrise text-[18px] mt-0.5 shrink-0">{item.icon}</span>
                <span className="font-body-md text-white/70">{f[item.key] ?? item.fallback}</span>
              </div>
            ))}
          </div>
      ...
      <div className="border-t border-white/5">
        <div className="max-w-7xl mx-auto px-container-padding-mobile md:px-container-padding-desktop py-6 flex flex-col sm:flex-row justify-between items-center gap-3">
          <p className="font-body-md text-white/50 text-sm text-center sm:text-left">
            {f.copyright ?? "© 2026 Hung Trinh AI. All rights reserved."}
          </p>
          <p className="font-label-eyebrow text-label-eyebrow text-white/40 uppercase tracking-wide sm:tracking-widest text-center sm:text-right">
            {f.tagline ?? "AI Training • Consulting • Automation"}
          </p>
        </div>
      </div>
