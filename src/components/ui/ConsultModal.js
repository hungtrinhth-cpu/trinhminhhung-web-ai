...
export default function ConsultModal({ isOpen, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] bg-black/50 flex items-center justify-center p-4 md:p-6"
      style={{ animation: "fadeIn 0.25s ease-out" }}
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div
        className="relative w-full max-w-[860px] rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row"
        style={{ animation: "scaleIn 0.35s cubic-bezier(0.16,1,0.3,1)" }}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/20 hover:bg-black/40 flex items-center justify-center text-white transition-colors"
        >
          <span className="material-symbols-outlined text-xl">close</span>
        </button>

        {/* Left panel */}
        <div className="bg-ink-text md:w-[340px] shrink-0 p-8 md:p-10 flex flex-col justify-between">
          <div className="space-y-6">
            ...
            <div>
              <p className="font-label-eyebrow text-label-eyebrow text-primary-container uppercase tracking-widest mb-3">
                Miễn phí · 30 phút
              </p>
              <h2 className="font-headline-section text-headline-section-mobile text-pure-white leading-tight uppercase">
                TƯ VẤN<br />
                <span className="text-primary-container">CHIẾN LƯỢC</span><br />
                AI
              </h2>
            </div>

            <p className="font-body-lg text-pure-white/60 leading-relaxed">
        ...
        {/* Right panel — form */}
        <div className="flex-1 bg-pure-white p-8 md:p-10">
          {submitted ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-8">
              ...
              <h3 className="font-headline-section text-headline-section-mobile text-ink-text">Đã nhận thông tin!</h3>
              <p className="font-body-lg text-slate-subtext max-w-xs">
                Chúng tôi sẽ liên hệ với bạn trong vòng 30 phút trong giờ làm việc.
              </p>
              <button
                onClick={onClose}
                className="mt-4 bg-primary-container text-white px-8 py-3 rounded-full font-button-text text-button-text hover:scale-105 transition-transform"
              >
                Đóng
              </button>
            </div>
            ...
            <>
              <div className="mb-6">
                ...
                <span className="font-headline-sub text-headline-sub text-ink-text align-middle">Thông Tin Liên Hệ</span>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <input
                  ...
                  className="w-full px-5 py-4 rounded-xl border border-border-subtle bg-mist-bg font-body-lg text-ink-text placeholder:text-slate-subtext/40 focus:outline-none focus:border-primary-container transition-all"
                />
                <input
                  required
                  type="tel"
                ...
                <textarea
                  ...
                  className="w-full px-5 py-4 rounded-xl border border-border-subtle bg-mist-bg font-body-lg text-ink-text placeholder:text-slate-subtext/40 focus:outline-none focus:border-primary-container transition-all resize-none"
                />
                <button
                  type="submit"
